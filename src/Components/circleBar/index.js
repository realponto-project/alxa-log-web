...
const CircleBar = ({ icon, total = 0, count = 0 }) => {
  ...
  const x = 46 + 46 * Math.cos(a)
  const y = 46 + 46 * Math.sin(a)
  let indicator = null

  if ((Math.PI * 2 * count) / total > Math.PI) {
    indicator = `M46,46 L46,0 A46,46 1 0,1 ${46},${92} A46,46 1 0,1 ${x},${y}  z`
  } else {
    indicator = `M46,46 L46,0 A46,46 1 0,1 ${x},${y} z`
  }
  ...
  return (
    <svg
      ...
      <path d={indicator} fill="url(#paint0_linear)"></path>
