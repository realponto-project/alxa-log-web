      ...
      <Row gutter={[8, 8]}>
            ...
            <Row style={{ padding: '0 16px 16px' }}>
              ...
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  Placa do veículo da manutenção
                </Text>
              </Col>
              <Col span={8}>
              ...
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>
                    {maintenanceOrder && maintenanceOrder.plateHorse}
                  </strong>
                </Text>
              </Col>
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>
                    {maintenanceOrder && maintenanceOrder.plateCart}
                  </strong>
                </Text>
              </Col>
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>
                    {maintenanceOrder && services[maintenanceOrder.service]}
                  </strong>
                </Text>
              </Col>
              ...
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>{permananceTimeDetail}</strong>
                </Text>
              </Col>
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>
                    {maintenanceOrder && parseStatus[maintenanceOrder.status]}
                  </strong>
                </Text>
              </Col>
              <Col span={8}>
                <Text style={{ color: '#FFFFFF' }}>
                  <strong>
                    {maintenanceOrder && status[maintenanceOrder.priority]}
                  </strong>
                </Text>
              </Col>
            </Row>
          ...
          <Row>
            <Col span={24}>
              <Divider orientation="left">
                <strong>
                  Dados do condutor da entrada{' '}
                  {!maintenanceOrder.driverSecondary &&
          ...
          maintenanceOrder.maintenanceOrderDrivers.length > 1 && (
            <Col span={24} style={{ padding: '16px' }}>
              <Row>
                <Col span={24}>
                  <Divider orientation="left">
                    <strong>Dados do condutor da saída</strong>
                  </Divider>
                </Col>
                <Col span={24}>
                  <Text>Nome</Text>
        ...
        <Col span={24} style={{ padding: '16px' }}>
          <Row>
            ...
            <Col span={24}>
              {maintenanceOrder && maintenanceOrder.supplies.length === 0 && (
                <h5>
                  Não tem nenhum abastecimento vinculado a essa ordem de
                  manutenção!
                </h5>
              )}
              {maintenanceOrder &&
                maintenanceOrder.supplies.map(
                  ({ id, fuel, km, totalLiters, odometer }) => (
        ...
        <Col span={24} style={{ padding: '16px' }}>
          <Row>
            ...
            <Col span={24}>
              <Row gutter={[8, 8]}>
                {maintenanceOrder &&
                  maintenanceOrder.maintenanceOrderEvents.map(
                    ({ createdAt, id, status }) => (
                      <Col span={8} key={id}>
                        <Text>{parseStatus[status]}</Text>
                        <br />
                        <Text>
                          <strong>
                            {maintenanceOrder &&
                              formattedDate(createdAt, 'DD MMM')}
                          </strong>
                        </Text>
                      </Col>
                    )
