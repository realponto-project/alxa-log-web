...
const formSettingsVehicle = (vehiclesSource) => [
  {
    ...
    rules,
    typeInput: 'select',
    options: vehiclesSource.map((item) => ({
      value: item.plate,
      label: item.plate
    }))
  },
  ...
  formSettingsVehicle(vehiclesSource).map((item) => {
    if (item.name === 'operationId') {
      return {
        ...item,
        show: true,
        options: operationsSource.map(parseOptionItemOperation)
      }
    }
    ...
    if (item.name === 'companyId') {
      return {
        ...item,
        show: true,
        options: branchsSource.map(parseOptionItem)
      }
    }
    ...
    if (item.name === 'driverId') {
      return {
        ...item,
        show: true,
        options: driversSource.map(parseOptionItemDriver)
      }
    }
...
export { settingsNextStep, formSettingsVehicle, formSettingsVehicleEdit }
